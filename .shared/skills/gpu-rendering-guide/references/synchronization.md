...
Unreal's timeline semaphores and RDG's automatic barrier insertion follow the same model: derive minimal scoped barriers from declared pass dependencies, and use a monotonic counter for cross-queue and CPU-side waits instead of ad-hoc binary semaphores everywhere.
