# texture-mapping-advanced: Advanced Texture Mapping
...
1. **Triplanar / biplanar weights** — Blend axis projections by normal or slope magnitude.
2. **No-tile sampling** — Hash per-cell UV offsets to break obvious texture repetition.
3. **Ray-differential filtering** — Scale mip/footprint from UV gradients for anisotropic surfaces.
4. **Material blending** — Composite rock/grass/snow by height and slope thresholds.
...
vec4 textureNoTileCheap(sampler2D sam, vec2 uv) {
    float k = texture(uChannel1, 0.005 * uv).x;  // low-freq variation index
    float index = k * 8.0;
    float i = floor(index);
    float f = fract(index);
...
- Triplanar weights without normalization sum > 1 at corners — normalize `w = abs(n)^k / dot(abs(n)^k, vec3(1))`.
- Object-space triplanar drifts when the mesh animates — use world-space position for moving objects.
- Parallax without limiting offset samples through floor — cap parallax steps and discard when UV leaves [0,1].
