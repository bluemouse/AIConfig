...
- **Execution-only barriers** - For a write-after-read (WAR) hazard where no availability/visibility is needed, an execution dependency (stage masks, empty access masks) suffices.
- **Queue-family transfer** - For a resource moving between queue families, emit a release barrier on the source (set `srcQueueFamilyIndex`/`dstQueueFamilyIndex`) and a matching acquire barrier on the destination, see [device-and-queues.md](./device-and-queues.md).
...
- Avoid `VK_IMAGE_LAYOUT_GENERAL` as a permanent convenience; use the use-specific layout except where `GENERAL` is genuinely required (e.g. storage image read+write).

## Related

[device-memory.md](./device-memory.md), [synchronization.md](./synchronization.md), [device-and-queues.md](./device-and-queues.md), [pipelines.md](./pipelines.md)
