...
the relevant section(s) right before running that scope's pass (at `deep` effort, the
checklist bullets get pasted verbatim into that scope's subagent prompt).
...
3. **Gather the diff and context** — run the resolved git command(s); read surrounding
   code, call sites, and existing tests as needed. If the source includes untracked files
   (target #3), capture them with `git status --porcelain` and read their contents — diffs
   alone never show untracked files.
4. **Run the review**, branching by effort:
   ...
   - `deep`: Scope & Intent Alignment (inline, by you — see next section for why), then
     parallel per-scope subagent dispatch, then dedup, then adversarial verify, then a
     gap-sweep pass.
...
prompt. Subagents do not load this skill or reference file.

**Diff handoff**: prefer giving subagents an exact git command to run (repo path +
resolved command from the source table) rather than pasting the full diff inline when the
patch is large (~800+ lines or ~40 KB). For smaller diffs, inline paste is fine. When
untracked files matter, list their paths and either paste their contents or instruct
subagents to read them.

**Dispatch**: in a single message, issue one Task tool call per *selected* scope, all in
parallel (multiple tool calls in one turn, not sequential calls). Configure each with:

- `subagent_type`: `generalPurpose` — Read/Grep/Shell access to inspect surrounding code
- `readonly`: `true` — reviewers must not modify code
- `run_in_background`: `false` — wait for all scopes to finish before dedup (in
  Multitask Mode, use `true` instead and proceed with dedup when notifications arrive)
- `description`: short label like `Code review: <scope> scope`

There is no specialized per-scope reviewer subagent type; `generalPurpose` is the right
default for every lens. Do not use `bugbot` or `security-review` here — those are
single-lens, diff-inferring shortcuts for `/review-bugbot` and `/review-security`, not
this multi-scope workflow.
...
<the full resolved diff if small, OR exact instructions to reproduce it, e.g. "run
`git diff --staged` in the repository above". If untracked files are in scope, list
their paths and contents or instruct the subagent to read them.>
...
Do not assign a final verdict (CONFIRMED/PLAUSIBLE/REFUTED) — that happens in a
separate verification pass you are not doing.
```

**Cross-scope reconciliation**: once all subagents return, collect findings from each
subagent's final message (you only receive final text, not intermediate tool output) and
dedupe by (file, overlapping line range, similar description). If two scopes flag
essentially the same underlying issue (e.g. `security` and `correctness` both flag the
same unvalidated input), merge into one finding, tag it with both scopes, and keep the
more severe rating.
...
in the main conversation (cap 6 candidates). If Task dispatch is unavailable entirely,
run all selected scopes sequentially instead. Note in the report when parallel dispatch or
individual scope subagents were unavailable.
...
running any scope's pass, and required verbatim in subagent prompts at `deep` effort.
