...
**Dispatch**: in a single message, issue one Agent-tool call per *selected* scope, all in
parallel (multiple tool calls in one turn, not sequential calls). Use `general-purpose` as
the subagent type — there's no specialized reviewer subagent type available here, and
general-purpose has the Read/Grep/Bash access each lens needs to inspect surrounding code.

**Subagent prompt template** — paste real content, not a pointer; the subagent doesn't
...
Call the ReportFindings tool once, at the end, with all candidates (file,
line, summary, failure_scenario, category: "<scope>"). Do not set verdict —
that happens in a separate verification pass you are not doing.

Then, in your final reply text (this is what actually reaches the
reviewer), restate each candidate as a markdown block with: Severity
(critical/important/suggestion), Location, What's wrong, Why it matters,
Proposed fix. ReportFindings' schema doesn't carry severity or a fix, so
this restatement is required, not optional.
```

**Why both `ReportFindings` and restated final text**: `ReportFindings` gives a
consistent, typed record of each subagent's raw candidates (file/line/summary/
failure_scenario/category) — a useful structured intermediate. But you only receive a
subagent's *final text message* back, not its tool calls. Since `ReportFindings`'s schema
has no severity, no fix field, and disallows setting a verdict before a verify pass, it
can't alone carry everything the Output Contract needs — hence requiring the richer
restatement in the final text too.

**Cross-scope reconciliation**: once all subagents return, collect all per-scope finding
...
- Posting findings as PR review comments is out of scope for this skill (unlike the
  built-in `/code-review --comment` flag) — if asked, say so.
