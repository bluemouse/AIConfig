...
- **Git workflow:** `commit-message-writer`, `git-guide`, `git-merge-guide`, `pull-request-guide`, `code-reviewer`, `techdoc-reviewer`, `github-guide` (craft: commit messages → git mechanics → deep local merge/rebase integration → PR authoring → diff review → documentation truth/sync when reader-visible contracts change; delivery on GitHub: `gh pr create`, post review, resolve threads; explicit `/` shortcuts: use **command-creator** for portable slash commands)
...
- **Codebase learning:** `code-professor`, `tutorial-writer`, `plan-guide`, `debugging-guide`, `code-reviewer` (evidence-backed onboarding and architecture guides → example-driven tutorials and user guides → plan changes → verified fixes → diff review; defers product research to `research-guide`)
...
- **Requirement clarification:** `prompt-clarifier` (with `research-guide` for open-ended discovery, `plan-guide` for planning, and `debugging-guide` for defect root cause — clarify blocking decisions, then hand off)
- **Decision deliberation:** `advisory-council`, `devil-advocate` (with `prompt-clarifier` when the decision framing is ambiguous, `research-guide` for open-ended discovery, and `plan-reviewer` for plan audit — council for multi-perspective synthesis with dissent; devil-advocate for single-proposal adversarial review with proceed/rework/reject verdict)
...
Example for painting-engine skills: `mypaint-engine-dev` and `krita-engine-dev` ship `eval-queries.json` under their bootstrap paths. `minutes-writer`, `commit-message-writer`, `code-reviewer`, `techdoc-reviewer`, `github-guide`, `command-creator`, `research-guide`, `research-reviewer`, `plan-guide`, `plan-reviewer`, `plan-executor`, `implementation-auditor`, `test-driven-dev-guide`, `debugging-guide`, `cpp-coding`, `cpp-performance-guide`, `agent-runner`, `prompt-clarifier`, `advisory-council`, `devil-advocate`, `qt-ui-guide`, and `tutorial-writer` ship `eval-queries.json` for description trigger testing. Kotlin/Gradle skills in `skills-ref/` also ship `eval-queries.json` for description trigger testing.
