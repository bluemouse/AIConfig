...
|---|---|---|---|
| Blocker | Execution would be unsafe, impossible, or likely to build the wrong thing. | Do not execute. | Missing source requirements, unresolved core decision, critical tasks vague, no verification for critical behavior, destructive migration without rollback, plan contradicts spec. |
| Major | Execution can start only with risky invention or significant clarification. | Revise before execution or accept explicitly as execution risk if narrow. | Missing file/interface details in known codebase, weak task sequencing, incomplete tests for core flow, unsupported architecture assumption, missing migration step. |
...
Check:
- Must-have requirements are mapped to tasks and verification.
...
- Verification matrix covers source requirements and high-risk tasks.
