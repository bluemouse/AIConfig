...
Unity's HDR Output pipeline and Unreal's HDR display path follow the same contract: linear scene rendering, explicit final encode to the swapchain color space, and paper-white scaling to the panel's measured peak — not the PQ theoretical maximum.
...
[references/shader-system.md](./shader-system.md), [references/command-recording-and-frames.md](./command-recording-and-frames.md)
