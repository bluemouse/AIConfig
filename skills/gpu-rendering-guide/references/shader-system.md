...
Unreal's shader permutation system and Unity's shader keyword/variant stripping follow the same tradeoff: compile specialized variants for hot paths, strip unused combinations at build time, and warm the pipeline cache before first draw to avoid hitches.
...
[references/binding-model.md](./binding-model.md), [references/render-graph.md](./render-graph.md), [references/command-recording-and-frames.md](./command-recording-and-frames.md), [references/render-pipeline-composition.md](./render-pipeline-composition.md)
