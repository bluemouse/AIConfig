...
Unreal's bindless resource model applies the same frequency principle: collapse per-material texture access into indexed arrays rather than per-draw binds. Unity's SRP Batcher is a related but distinct idea — batch draws that share shader variants and compatible constant-buffer layout so per-object data updates without rebinding the pipeline.
...
[references/shader-system.md](./shader-system.md), [references/synchronization.md](./synchronization.md), [references/command-recording-and-frames.md](./command-recording-and-frames.md), [references/gpu-memory-strategy.md](./gpu-memory-strategy.md), [references/scene-rendering-culling.md](./scene-rendering-culling.md)
